...
\def\connectH#1#2#3{\draw (#1) -- ++(0,#3) coordinate (inter) -| (#2 |- inter) -- (#2);}
...
\begin{tikzpicture}
  ...
  \draw (ard D03) -- ++(-2.5,0) -- ++(0,8) coordinate (inter2) to[short,-*] (inter |- inter2);
  \connectV{PISO1 CLK}{PISO2 CLK}{-1.5}
  \draw (ard D02) -- ++(-3,0) -- ++(0,8) coordinate (inter2) to[short,-*] (inter |- inter2);

  \draw (PISO1 SER) -- ++(8,0) coordinate (inter) -- (inter |- ard D04) -- (ard D04);
  ...
  \draw (ard 5V) -- (jingle VCC 4 -| ard 5V) -- (jingle VCC 4);
